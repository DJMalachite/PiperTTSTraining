...
def _hardware(prof: profile_mod.Profile, split) -> None:
    ...
    if info.compiled_for_device is False:
        # The expensive failure: everything reports fine and the run dies at the
        # first elementwise kernel, an hour in. Cheaper to say so here.
        tui.info("")
        tui.warn(
            f"this torch has no {env_mod.normalise_gfx(info.gcn_arch)} kernels "
            f"(built for {', '.join(info.arch_list) or 'unknown'}), so training "
            f"will fail even though the GPU reports as available"
        )
        tui.hint("  " + env_mod.unsupported_arch_advice(info))
